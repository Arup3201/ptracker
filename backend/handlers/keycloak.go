...
func (handler *KeycloakHandler) KeycloakRefresh(w http.ResponseWriter, r *http.Request) error {
	...
	if err != nil {
		return &HTTPError{
			Code:    http.StatusInternalServerError,
			Message: "Token refresh error",
			Err:     fmt.Errorf("keycloak refresh token: access token store: %w", err),
		}
	}
...
func (handler *KeycloakHandler) KeycloakLogout(w http.ResponseWriter, r *http.Request) error {
	...
	_, err = handler.Redis.Set(context.Background(), "access_token:session:"+sessionId, "", 0).Result()
	if err != nil {
		return &HTTPError{
			Code:    http.StatusInternalServerError,
			Message: "Logout error",
			Err:     fmt.Errorf("keycloak logout: access token removal: %w", err),
		}
	}
